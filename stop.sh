...
PIDS=$(ps aux | grep -E "hivematrix-(core|nexus|helm|brainhair|codex|knowledgetree|ledger|archive|beacon)" | grep -v grep | awk '{print $2}')

if [ -n "$PIDS" ]; then
    echo -e "${YELLOW}Found lingering processes:${NC}"
    ps aux | grep -E "hivematrix-(core|nexus|helm|brainhair|codex|knowledgetree|ledger|archive|beacon)" | grep -v grep | awk '{print "  PID " $2 ": " $11}'
    echo ""
