"""
Service-to-Service Authentication Helper for HiveMatrix Helm

Usage:
    from app.service_client import call_service

    response = call_service('codex', '/api/data')
"""
...
def call_service(service_name, path, method='GET', **kwargs):
    ...
    Args:
        service_name: The target service name (e.g., 'codex', 'ledger')
    ...
    Example:
        response = call_service('codex', '/api/search', method='POST', json={'q': 'test'})
        data = response.json()
    """
    ...
    calling_service = current_app.config.get('SERVICE_NAME', 'helm')
