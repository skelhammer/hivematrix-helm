...
def init_jwks_client():
    ...
    if not core_url:
        raise ValueError("CORE_SERVICE_URL not configured")

    try:
        jwks_client = jwt.PyJWKClient(f"{core_url}/.well-known/jwks.json")
        print(f"✓ Connected to Core service at {core_url}")
    except Exception as e:
        raise RuntimeError(f"Could not connect to Core service at {core_url}: {e}")

def token_required(f):
    """
    A decorator to protect routes, ensuring a valid JWT is present.
    Accepts both user tokens and service tokens.
    """
    @wraps(f)
...
def admin_required(f):
    ...
    @token_required
    def decorated_function(*args, **kwargs):
        # In dev mode (no Core), g.user will already be set to admin
